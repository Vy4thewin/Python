...
def exercise01():
    ...
    df=pd.DataFrame([ [0,1,2 ,3],[0,1,2 ,3 ],[4,1 ,5 ,6] ])
    return df
...
def exercise11(n):
    ...
    '''
    #.arange makes a array from 0 to n-1. reshape take a tuple ( rows,columns)
    array_1d=np.arange(n)
    array_reshaped=array_1d.reshape((3,math.floor(n/3))) 
...
def exercise14(words):
    ...
    '''
    #for x in dataframe, find its length and assign to new column length
    df=pd.DataFrame(words)
    df["length"]=df[0].map(lambda x: len(x))
    return df
